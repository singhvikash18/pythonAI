...
def reply(text_version):
  ...
  if "adam" in text_version:
    ...
    if keyword != '':
      ...
      exit()
    if keyword !="song":
      url = "https://soundcloud.com/"  + keyword
      speak("here we go" + keyword)
      webbrowser.open(url)
      sleep(2)
      exit()
  if "morning" in text_version:
      speak("good morning ")  
      
  if "stop" in text_version or "exit" in text_version:
...
sleep(1)
while True:
  speak("welcome, What do you want")
